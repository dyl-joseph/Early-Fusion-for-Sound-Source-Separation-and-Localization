...
class Resnet(nn.Module):
    def __init__(self):
        ...
        self.model.blocks[5].proj = nn.Linear(2048, 512)
        
        for param in self.model.parameters():
            param.requires_grad = False
        
        for param in self.model.blocks[5].proj.parameters():
            param.requires_grad = True

    def forward(self, video_frames):
        video_vector = self.model(video_frames)
        # print(video_vector.shape)
        video_feat_expanded = video_vector.unsqueeze(-1).unsqueeze(-1)


        return video_feat_expanded
...
class AudioVisual7layerUNet(nn.Module):
    ...
    def forward(self, x, visual_feat):
        ...
        print(f"audio_conv1feature shape: {audio_conv1feature.shape}")
        audio_conv2feature = self.audionet_convlayer2(audio_conv1feature)
        print(f"audio_conv2feature shape: {audio_conv2feature.shape}")
        audio_conv3feature = self.audionet_convlayer3(audio_conv2feature)
        print(f"audio_conv3feature shape: {audio_conv3feature.shape}")
        audio_conv4feature = self.audionet_convlayer4(audio_conv3feature)
        print(f"audio_conv4feature shape: {audio_conv4feature.shape}")
        audio_conv5feature = self.audionet_convlayer5(audio_conv4feature)
        print(f"audio_conv5feature shape: {audio_conv5feature.shape}")
        audio_conv6feature = self.audionet_convlayer6(audio_conv5feature)
        print(f"audio_conv6feature shape: {audio_conv6feature.shape}")
        audio_conv7feature = self.audionet_convlayer7(audio_conv6feature)
        print(f"audio_conv7feature shape: {audio_conv7feature.shape}")

        # print(f"audio features: {audio_conv7feature.shape}")
        # print(f"visual features: {visual_feat.shape}")
        audioVisual_feature = torch.cat((visual_feat, audio_conv7feature), dim=1)
        
        # print(f"combined values: {audioVisual_feature.shape}")
...
class AudioVisual5layerUNet(nn.Module):
    ...
    def forward(self, x, visual_feat):
        ...
        audio_conv4feature = self.audionet_convlayer4(audio_conv3feature)
        audio_conv5feature = self.audionet_convlayer5(audio_conv4feature)
        # print(f"audio features: {audio_conv5feature.shape}")
        # print(f"visual features: {visual_feat.shape}")
        visual_feat = visual_feat.expand(-1, -1, 8, 8) # tile video feature
