...
# train one epoch
def train(netWrapper, loader, loader_val, optimizer, history, epoch, args, nets):
...
def checkpoint(nets, history, epoch, args):
    ...
    global_ckpt_dir = '/home/dylan_j/Documents/sound_diar/global_checkpoints'
    os.makedirs(global_ckpt_dir, exist_ok=True)

    torch.save(history,
               '{}/history_{}'.format(global_ckpt_dir, suffix_latest))
    torch.save(net_sound.state_dict(),
               '{}/sound_{}'.format(global_ckpt_dir, suffix_latest))
    torch.save(net_frame.state_dict(),
               '{}/frame_{}'.format(global_ckpt_dir, suffix_latest))
    ...
    if history['val']['err']:
        ...
        if cur_err < args.best_err:
            ...
            torch.save(net_sound.state_dict(),
                       '{}/sound_{}'.format(global_ckpt_dir, suffix_best))
            torch.save(net_frame.state_dict(),
                       '{}/frame_{}'.format(global_ckpt_dir, suffix_best))
            # torch.save(net_synthesizer.state_dict(),
...
def main(args):
    ...
    for epoch in range(1, args.num_epoch + 1):
        train(netWrapper, loader_train, loader_val, optimizer, history, epoch, args, nets)
